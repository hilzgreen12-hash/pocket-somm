# Automated Code Review — 2026-05-17

> Issues carried forward from previous reviews (unresolved as of 2026-05-16) are marked **[UNRESOLVED]**.
...
### HIGH — `app/(tabs)/history.tsx:71` — `recommendation.topPick` does not exist in `RecommendationResponse` [NEW]
...
`RecommendationResponse` (defined at `src/types/wine.ts:33–36`) has only `wines: WineRecommendation[]` and `summary: string`. There is no `topPick` property. This expression always evaluates to `undefined`, so the wine name line never renders on any history card. This is a silent display regression — users see dates and restaurant names but the top recommendation is always absent.
...
### MEDIUM — `app/scan/camera.tsx:29–99` — `handleCapture` has no try/catch; unhandled rejection on camera or disk failure [NEW]
...
### LOW — `src/api/claude.ts:1` — `supabase` import unused [NEW]
...
### LOW — `app/scan/extracting.tsx:139–161` — No cancel button during extraction; users cannot abort a stuck operation [NEW]

The extracting screen instructs users "Please don't leave this page" but provides no cancel/abort button. The `token` object (line 65) prevents state updates after unmount but does not cancel the in-flight `fetch` calls. Combined with the missing fetch timeout (see above), a user who wants to abort — because they scanned the wrong list, changed tables, or the spinner has been running for two minutes — has no recourse other than force-quitting the app.
...
### LOW — `app/scan/preferences.tsx` — `/scan/preferences` route is unreachable in the current navigation flow [NEW]
...
|------|------|--------|-----|-------|
| Bugs and Crashes | 6 | 7 | 0 | 13 |
| Supabase / Edge Functions | 3 | 3 | 3 | 9 |
| UX and Performance | 0 | 3 | 6 | 9 |
| Navigation | 0 | 2 | 4 | 6 |
| **Total** | **9** | **15** | **13** | **37** |

**New this review:** 4 findings (1 High, 2 Medium, 1 Low in Bugs; 1 Low in Supabase; 1 Low in UX; 1 Low in Navigation).

**Persistent critical items (High, 6+ reviews unresolved):** the three edge functions accept unauthenticated requests using only the public anon key, the OCR function is vulnerable to SSRF via the `url` parameter, and the app has no root error boundary. These represent the highest-risk items in the codebase and should be prioritised before any production launch.
