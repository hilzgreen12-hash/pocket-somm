...
No application code has been committed since the initial codebase. All findings from the 2026-06-18 report carry forward unchanged. This pass adds two new findings not present in any prior report.
...
```
If a user uploads four images and one fails to parse (blurry photo, API timeout, unexpected response format), `Promise.all` rejects immediately and discards all successfully extracted wines from the other three images. The user sees the generic "No wines were detected" error with no indication that three of their four uploads worked. Fix: replace `Promise.all` with `Promise.allSettled`, filter for fulfilled results, merge those wines, and only show an error if zero images produced any output. Surface a partial-success notice when fewer than all images succeeded.
...
```
`prefs` is `userProfile` (the Supabase saved preferences). `preferences.budget` from the scan store (set in `scan.tsx` when the user adjusts the slider before tapping "Scan Wine List") is not used. A scan-level budget raise or override is ignored and wines are discarded before reaching the recommender.
...
```
`wineName` is correctly URL-encoded with `encodeURIComponent`, but `vintageParam` is interpolated verbatim. A caller with the anon key can pass a crafted `vintage` value such as `"NV&format=csv&another=injected"` to append arbitrary query parameters to the Wine-Searcher API request, potentially returning different wine data that gets written to `pricing_cache`. Fix: `const vintageParam = vintage != null ? encodeURIComponent(String(vintage)) : 'NV'`.
...
**M18 — `AsyncStorage.getItem` in app entry-point has no error handler; blank screen on storage failure** *(new)*
`app/index.tsx:13`:
```tsx
AsyncStorage.getItem('hasLaunched').then((v) => setHasLaunched(v === 'true'));
```
There is no `.catch()` on this promise. If AsyncStorage encounters a read error (corrupted storage, full disk, or platform-level exception), the promise rejects silently, `hasLaunched` stays `null` permanently, and line 16 returns `null` rendering a blank screen with no recovery path. Add a `.catch(() => setHasLaunched(false))` so that any storage failure falls through to the welcome screen.

**M19 — `getSession()` in `AuthProvider` has no error handler; auth loading state stuck permanently on network failure** *(new)*
...
```
There is no `.catch()` on this call. If `getSession()` throws — due to a network error, an expired or corrupted token in `SecureStore`, or a Supabase SDK exception — `setLoading(false)` is never called. `loading` stays `true` permanently. `app/index.tsx:16` returns `null` while `loading` is `true`, so the app renders a blank screen that persists until the user force-quits. Fix: add `.catch(() => setLoading(false))` so that failed auth initialization falls through to the unauthenticated state.
