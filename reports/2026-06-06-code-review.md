...
Zero issues from the 2026-06-05 report have been resolved. No code commits exist since the initial codebase was created; all carry-forward findings remain open. Three new issues are identified in this pass.
...
**H7 — `handleEmailChange` in profile leaves `emailSaving` permanently true if `updateUser` throws** *(carry-forward)*
...
async function handleEmailChange() {
  ...
  setEmailSaving(false); // never reached if updateUser throws
  ...
}
```
If `supabase.auth.updateUser` throws (network timeout, SDK error), `setEmailSaving(false)` is never called. The Confirm button renders an `ActivityIndicator` permanently and the email-change UI is frozen until the app is killed. Fix: wrap in `try/finally` with `setEmailSaving(false)` in the `finally` clause.

**H8 — `usePreferences` `mutationFn` silently discards Supabase upsert errors; `onError` never fires** *(new)*
...
```
The Supabase JS v2 client does not throw on query failures — it returns a resolved promise with a non-null `error` field. Because the return value of `supabase.from('profiles').upsert(...)` is not destructured and the `error` field is never checked, `mutationFn` always resolves successfully regardless of whether the database write succeeded. React Query therefore never calls `onError`. The `onSuccess` handler at line 49 fires, invalidates the query cache, and the UI behaves as if the save succeeded. The error is not logged, not shown, and the failed write is completely invisible. Users editing preferences on the profile tab or completing onboarding believe their changes were persisted when they were not. This is more severe than previously described in M6, which implied `onError` would at least log errors — in practice `onError` is never reached. Fix: destructure `const { error } = await supabase.from(...)`, check `if (error) throw error`, then `onError` will fire correctly.
...
**M6 — Preference save errors are invisible to user and not logged** *(carry-forward — see H8 for root cause)*
`src/hooks/usePreferences.ts:50`: `onError: (err) => console.error(...)`. As detailed in H8, this callback is never reached because `mutationFn` never throws. A Supabase upsert failure (network error, RLS rejection, schema mismatch) is completely invisible — not logged, not alerted.
...
```
`data` is not destructured. When a Supabase project has email confirmation disabled (a valid production configuration), `signUp()` returns `data.session` as a live, valid session immediately. The current code ignores this, always shows "Check your email", and routes to sign-in — forcing the user to enter their credentials a second time. Fix: destructure `data` alongside `error`; if `data.session` is non-null, store the session and navigate to `/(tabs)/scan` directly rather than showing the email-confirmation dialog.
...
**L8 — `invokeFunction` calls `JSON.parse(text)` without try/catch; gateway errors produce raw SyntaxError messages** *(carry-forward)*
...
**L9 — `WineRecommendationCard` component is dead code; results screen re-implements its layout inline** *(carry-forward)*
`src/components/results/WineRecommendationCard.tsx` (196 lines) is defined but never imported by `app/scan/results.tsx` or any other file. The results screen (`app/scan/results.tsx:49–115`) re-implements the wine card layout inline with a different accordion design. Two divergent representations of the same data now exist and must be kept in sync independently. Either delete `WineRecommendationCard.tsx` or refactor `results.tsx` to use it.

**L10 — `profiles.updated_at` is never updated on upserts; column reflects creation time only** *(carry-forward)*
`supabase/migrations/001_initial_schema.sql:7` defines `updated_at timestamptz default now()`. No `before update` trigger sets `updated_at = now()` on row modification. `src/hooks/usePreferences.ts:38–47` upserts preference data without including `updated_at` in the payload. Every profile row shows its original creation timestamp for `updated_at` regardless of how many preference changes the user has made. Fix: add a `before update` trigger (`moddatetime` extension or a custom function) or include `updated_at: new Date().toISOString()` in every upsert call.

**L11 — `handleScan` and `handleScreenshot` lack double-tap guards** *(new)*
`app/(tabs)/scan.tsx:81–83` and `86–101`:
```tsx
function handleScan() {
...
async function handleScreenshot() {
  ...
  if (!result.canceled && result.assets.length > 0) {
    ...
    router.push('/scan/preview'); // or router.push('/scan/extracting')
  }
}
```
Neither function has an `isNavigating` ref, a `disabled` state, or any guard against concurrent calls. A rapid double-tap on "Scan Wine List" pushes `/scan/camera` twice onto the navigation stack; a fast double-tap on "Upload Screenshot" after image selection similarly pushes the destination route twice. The user must press back twice to return to the scan tab. This is the same class of defect as M8 (camera capture button double-tap) but affects the two primary entry-point buttons on the scan tab. Fix: set a `navigating` ref to `true` before the push and reset it on return, or disable the buttons during async work.

**L12 — `CameraOverlay` computes frame dimensions from stale `Dimensions` at module init time** *(new)*
`src/components/scan/CameraOverlay.tsx:4–5`:
```ts
const { width } = Dimensions.get('window');
const FRAME_WIDTH = width * 0.9;
const FRAME_HEIGHT = FRAME_WIDTH * 1.4;
```
`Dimensions.get('window')` is called once at module load. On iPad in split-view or when the device is rotated (landscape mode), `FRAME_WIDTH` and `FRAME_HEIGHT` remain fixed at the dimensions from the first load, causing the corner-bracket guides to visually misalign with the actual frame boundaries. The crop calculation in `camera.tsx:50` calls `Dimensions.get('window')` at capture time (correct) and uses the `frameRect` from `onLayout` (correct), so the crop math is unaffected — but the displayed overlay guide is rendered at the wrong size for the current orientation. Fix: move the dimension computation inside the component and replace `Dimensions.get` with `useWindowDimensions()`.
