...
No application code has been committed since the initial codebase. All findings from the 2026-06-17 report carry forward unchanged. This pass adds four new findings not present in any prior report.
...
**H6 — `Promise.all` in multi-image OCR discards all results if any single image fails** *(new)*
`app/scan/extracting.tsx:77`:
```tsx
const results = await Promise.all(imageUris.map(extractWineList));
```
If a user uploads four images and one fails to parse (blurry photo, API timeout, unexpected response format), `Promise.all` rejects immediately and discards all successfully extracted wines from the other three images. The user sees the generic "No wines were detected" error with no indication that three of their four uploads worked. This is a regression for the multi-page wine-list use case, which is a primary flow. Fix: replace `Promise.all` with `Promise.allSettled`, filter for fulfilled results, merge those wines, and only show an error if zero images produced any output. Surface a partial-success notice when fewer than all images succeeded.
...
`app/(auth)/sign-in.tsx:12–20` and `app/(auth)/sign-up.tsx:12–22`: if `signInWithPassword` or `signUp` throws (network timeout, SDK error), `setLoading(false)` on the next line is never reached. The button stays in its loading state permanently and the form is frozen until the app is killed. Move `setLoading(false)` into a `finally` block in both files.
...
**M6 — Upsert errors never reach `onError`; preferences silently not saved** *(carry-forward, escalated)*
...
```
`prefs` is `userProfile` (the Supabase saved preferences). `preferences.budget` from the scan store (set in `scan.tsx` when the user adjusts the slider before tapping "Scan Wine List") is not used. A scan-level budget raise is ignored and wines are discarded before reaching the recommender.
...
`src/hooks/usePreferences.ts:50`: `onError: (err) => console.error(...)`. As noted in M6, this path is never reached anyway. But even if it were, a silent console log is never seen by the user.
...
`app/(auth)/sign-up.tsx:13`: `data` is not destructured. When the Supabase project has email confirmation disabled, `signUp()` returns a live session in `data.session`. The current code always shows "Check your email" and routes to sign-in, forcing users to re-enter their credentials.

**M13 — `handleEmailChange` leaves `emailSaving` permanently true if `updateUser` throws** *(carry-forward)*
`app/(tabs)/profile.tsx:110–128`: Same pattern as M2. If `supabase.auth.updateUser` throws, `setEmailSaving(false)` is skipped. The Confirm button shows a permanent `ActivityIndicator` until the app is killed. Move `setEmailSaving(false)` into a `finally` block.
...
**M16 — URL injection via unencoded `vintage` parameter in wine-searcher-proxy** *(new)*
...
```
`wineName` is correctly URL-encoded with `encodeURIComponent`, but `vintageParam` is interpolated verbatim. Any caller with the anon key can pass a crafted `vintage` value such as `"NV&format=csv&another=injected"` to append arbitrary query parameters to the Wine-Searcher API request. While the Wine-Searcher API key is not exposed to the caller, an attacker could manipulate the query to return different wine data, causing poisoned entries to be written to `pricing_cache`. Fix: encode the vintage value — `const vintageParam = vintage != null ? encodeURIComponent(String(vintage)) : 'NV'`.
...
```
This creates a window where `local` and `selected` disagree. When `onChange` triggers a Supabase upsert that fails silently (M6), React Query re-fetches and sets the prop back to the old value, causing the UI to snap back after the user's toggle. Prefer fully controlled behaviour: remove `local`, use `selected` directly, and call `onChange` without caching state.
...
`app/_layout.tsx:15`: `Font.useFonts` returns a tuple `[boolean, Error | null]`. The error element is discarded. If any font file fails to load, `fontsLoaded` stays `false` permanently and the app renders `null`, displaying a blank screen with no recovery path. Destructure and handle the error value.

**L2 — `defaultBudget` type mismatch between interface and runtime value** *(carry-forward)*
`src/types/preferences.ts:7` declares `defaultBudget: number` (non-nullable). `src/hooks/usePreferences.ts:26` returns `defaultBudget: data.default_budget ?? null`. TypeScript strict null checks are silenced by the `as UserPreferences` cast, hiding every nullable access.
...
`src/api/claude.ts:17`: If the edge function returns non-JSON (Cloudflare 502, Supabase maintenance page), `JSON.parse` throws a `SyntaxError` with the raw HTML as the message. The user sees "SyntaxError: Unexpected token '<', '<!DOCTYPE...' is not valid JSON". Wrap in try/catch and throw a user-friendly message.
...
**L12 — `£` hardcoded in client-side price display; `wine.currency` field ignored** *(new)*
`app/scan/results.tsx:82`:
```tsx
{wine.menuPrice != null && (
  <Text style={styles.price}>£{wine.menuPrice}</Text>
)}
```
Also `src/components/results/WineRecommendationCard.tsx:55`. The OCR function extracts a `currency` field per wine (`default: 'GBP'`). For menus priced in EUR, USD, or other currencies, the displayed symbol is always `£` regardless of what OCR detected. Use `wine.currency` to select the appropriate symbol or pass it through as a prefix/suffix.

**L13 — `defaultCurrency` field in `UserPreferences` interface is never stored or populated** *(new)*
`src/types/preferences.ts:8` declares `defaultCurrency: string`. This field does not exist in any database migration, is never included in the `upsert` payload in `usePreferences.ts`, and is never returned by the `queryFn` at line 23–32 of the same file. The `as UserPreferences` cast on line 31 silences the TypeScript error. Any future code that reads `preferences.defaultCurrency` will get `undefined` at runtime while TypeScript reports it as a `string`. Either add the column to the schema and wire it through, or remove the field from the interface.
