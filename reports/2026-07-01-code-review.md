...
**Status:** No application code has changed since the initial commit. All findings from the 2026-06-30 report remain open. This review adds one new finding (L11) and carries forward all prior unresolved issues with verified file paths and line numbers.
...
In PostgreSQL RLS, the `USING` expression applies to `SELECT`, `UPDATE`, and `DELETE`. For `INSERT`, only the `WITH CHECK` expression is consulted. Because neither policy specifies `WITH CHECK`, the effective insert check is `WITH CHECK (TRUE)` — any authenticated user can insert a `profiles` or `scan_sessions` row with a `user_id` belonging to another user, overwriting their profile on the next `upsert`.

Fix: add `with check (auth.uid() = user_id)` to both policies.

---

**H10 — `scan/preferences.tsx` is missing five required `RecommendInput` fields; enabling route N2 would cause an immediate build failure (carry-forward)**
...
`RecommendInput` (`src/services/recommender.ts:5–15`) declares `wineTypes`, `favouriteRegions`, `favouriteGrapes`, `dislikedRegions`, and `dislikedGrapes` as required fields. All five are absent. This is a TypeScript compile error currently hidden only because the route is orphaned (N2). If anyone wires `/scan/preferences` into the navigation flow, the build will fail to compile. Additionally, these fields being `undefined` at runtime in a non-strict build would cause the recommender to silently drop colour restriction, regional exclusions, and grape exclusions.
...
`signOut()` returns `{ error }`, which is not checked. If it fails (network error, expired token), navigation still proceeds. The session token remains in SecureStore and the user appears logged in on the next cold start.
...
Neither `handleSignIn` nor `handleSignUp` validates inputs before calling Supabase. Submitting a blank email and password surfaces Supabase's raw internal error string (e.g. `"You must provide either an email or phone number and a password"`) in an `Alert`.
...
If a Cloudflare 502 or Supabase maintenance page is returned, `JSON.parse` throws a `SyntaxError` whose message (`"Unexpected token '<', '<!DOCTYPE...' is not valid JSON"`) propagates through to the user-visible error detail in `extracting.tsx:120`.
...
**L9 — `hasDuplicateGrapes` only splits on `/`; blend separators `,`, `-`, and `&` are not handled (carry-forward)**
...
Common blend notations include `"Cabernet Sauvignon, Merlot"`, `"Cabernet Sauvignon-Merlot"`, and `"Grenache & Syrah"`. A wine described as `"Cabernet Sauvignon, Merlot"` is treated as a single unique grape string. If two different blends share the same primary variety but use different separators, the duplicate check produces a false negative.
...
`focusPoint` is never passed as a prop to `CameraView`. The `expo-camera` `CameraView` component accepts a `focusPoint` prop that moves the camera's focus region; without it, `autofocus="on"` keeps the camera in continuous-autofocus mode regardless of where the user taps. The visual tap gesture fires but the camera behaviour is identical whether the user taps or not.

Fix: add `focusPoint={focusPoint}` to the `CameraView` props and set `autofocus="off"` so focus only locks when the user taps.

---

**L11 — `handleRetake` in preview screen triggers a double navigation to `/(tabs)/scan` (new)**
`app/scan/preview.tsx:16–19,10–12`
...
`reset()` sets `imageUri` to `null` in the Zustand store. On the next render cycle, the `useEffect` observes `imageUri` becoming `null` and fires a second `router.replace('/(tabs)/scan')`. This produces two concurrent `router.replace` calls to the same destination — the effect is usually benign since both navigate to the same screen, but it pollutes the navigation event queue and can interact poorly with route guards or back-press handlers that check navigation state. Under concurrent React mode it can also trigger redundant layout work.

Fix: remove the explicit `router.replace` from `handleRetake` and rely solely on the `useEffect` guard, or remove the `useEffect` and handle both the direct call and the retake explicitly.
...
The recommend prompt injects `£${budget}` regardless of currency. The results screen always renders `£{wine.menuPrice}`. The OCR function correctly extracts a `currency` field, but this is ignored downstream. EUR and USD menus receive a budget constraint in the wrong currency.
...
When `stage === 'recommending'`, line 148 renders "Scoring by critic rating, vintage quality and value" and lines 150–152 render "This may take a minute or two" as a second, unconditional `<Text>` element — both visible at the same time alongside the stage title. Remove the duplicate copy.
...
*Automated review — 2026-07-01. No application code has changed since the initial commit. All findings above remain open.*
