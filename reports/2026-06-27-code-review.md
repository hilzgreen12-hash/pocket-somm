...
**Status:** No application code has been changed since the initial commit. Every finding from prior reports remains open. This review identifies four new issues not previously reported and carries forward all unresolved high- and medium-severity findings with verified file paths and line numbers.
...
**H8 — `usePreferences` query error returns `null`, incorrectly triggering onboarding for existing users (new)**
...
`null` is returned when a database error occurs and also when no profile row exists. These two cases are indistinguishable at the call site. An authenticated user who has already completed onboarding will be silently redirected to `/onboarding` every cold start whenever a transient Supabase error occurs — losing their current position and being asked to re-configure their profile. The M2 carry-forward bug describes the loading-state path (`undefined`) sending users to scan; this is the inverse: the error path (`null`) sending existing users to onboarding.

Fix: distinguish the two cases. One approach: return a sentinel value (e.g. `{ error: true }`) instead of `null` on query failure, and handle it separately in `index.tsx`.
...
`isError` is not destructured from `useQuery`. When the Supabase query fails, `isLoading` becomes `false` and `sessions` is `undefined`. The component falls through to the `!sessions?.length` branch and renders "No scans yet" — misleading copy that implies the user simply hasn't scanned yet, hiding the actual error. Users have no way to know whether their history failed to load or is genuinely empty.

Fix: destructure `isError` and render a distinct error state with a retry affordance when `isError` is `true`.

---

**M13 — Results screen renders no wine cards with no empty-state message when `wines` array is empty (new)**
`app/scan/results.tsx:49–115`

The Zod schema at `src/services/recommender.ts:56` validates `wines` as `z.array(WineRecommendationSchema).max(3)`, which accepts an empty array as valid. If the recommender returns zero wines (possible when M10 is triggered or the model cannot find qualifying options), the results screen renders the header, summary, and "Start Another Search" button — but no wine cards and no empty-state message explaining why. The user is left on a screen that looks incomplete with no actionable feedback.

Fix: add an explicit check for `recommendation.wines.length === 0` and render a message such as "No qualifying wines were found on this list with your current preferences."
...
`handleAddCustomDislikedRegion` (line 88) and `handleAddCustomDislikedGrape` (line 96) check `current.includes(trimmed)` but not `current.length >= 5`, unlike `handleAddCustomRegion` (line 75) and `handleAddCustomGrape` (line 83) which check both. The `ChipPicker` components for dislikes are rendered with `max={5}`, but users can bypass this cap by typing into the free-text field. The disliked lists can grow unbounded through the text input path.
...
**L8 — `UserPreferences.defaultCurrency` declared in type but has no database column and is never populated (new)**
`src/types/preferences.ts:6` · `src/hooks/usePreferences.ts:16–31` · `supabase/migrations/001_initial_schema.sql`

The `UserPreferences` interface declares `defaultCurrency: string` at line 6. No `default_currency` column exists in the `profiles` table (checked against all three migrations). The `usePreferences` query does not select any currency column, and the returned object never sets `defaultCurrency`. Any code path that reads `preferences.defaultCurrency` receives `undefined`. This field appears to represent planned-but-incomplete multi-currency support that was abandoned without cleaning up the interface.

Fix: either add the database column and wire it through the query, or remove the field from the `UserPreferences` interface and address multi-currency as a separate tracked task.
...
When `stage === 'recommending'`, line 148 renders "Scoring by critic rating, vintage quality and value" and lines 150–152 render "This may take a minute or two" as a second, unconditional `<Text>` element — both visible at the same time alongside the stage title. The duplicate copy is confusing.
...
**U9 — `prefsLoaded` guard prevents budget slider from reflecting profile default on first load (new)**
...
`savedPreferences` is `undefined` at the time `useState` initialises (React Query is async). The initial state is always `null`. The `useEffect` at line 59 corrects this once `savedPreferences` loads — but only if `!prefsLoaded`. On first render, before the effect runs, `BudgetSlider` renders with `value={null}`. Depending on how `BudgetSlider` handles a `null` prop, this can display an incorrect default position in the slider UI for a brief moment, or permanently if the effect fires before the component fully mounts on slower devices. The `prefsLoaded` guard is a fragile workaround for the `useState` initialisation timing problem; using `savedPreferences?.defaultBudget ?? null` directly as the slider value (without local state) would be simpler and correct.
...
*Automated review — 2026-06-27. No application code has changed since the initial commit. All findings above remain open.*
