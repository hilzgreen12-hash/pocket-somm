...
No application code has been committed since the initial codebase was created. The only commits since the last review are automated reports. All findings from the 2026-06-21 report carry forward unchanged. This pass adds two new findings not present in any prior report.
...
async function handleSignUp() {
  ...
  setLoading(false);
```
`setLoading(true)` is completely absent from `handleSignUp`. The `loading` state is initialised to `false` and never set to `true`. Consequences: (1) the button label never changes to "Creating account…" while the request is in-flight, giving the user no indication that anything is happening; (2) `disabled={loading}` on the button never activates, so rapid repeated taps can fire multiple parallel `signUp` calls against Supabase before the first resolves. Fix: add `setLoading(true)` as the first statement in `handleSignUp` and move `setLoading(false)` into a `finally` block.
...
```
The return value `{ data, error }` is discarded. The `mutationFn` never throws on Supabase errors, so React Query calls `onSuccess` regardless of whether data was saved. The `onError` handler at line 50 never fires. Fix: destructure `const { error } = await supabase.from('profiles').upsert(...)` and `if (error) throw error`.
...
```
This creates a window where `local` and `selected` disagree. When `onChange` triggers a Supabase upsert that fails silently (M7), React Query re-fetches and resets the prop to the old value, causing the UI to snap back after the user's toggle. Prefer fully controlled behaviour: remove `local`, use `selected` directly, and call `onChange` without caching state.

**M19 — `AsyncStorage.getItem` in app entry-point has no error handler; blank screen on storage failure** *(carry-forward)*
`app/index.tsx:13`:
```tsx
...
**M20 — `getSession()` in `AuthProvider` has no error handler; auth loading state stuck permanently on network failure** *(carry-forward)*
...
```
`focusPoint` is computed and stored on every tap but is never passed to `CameraView`. The component renders `<CameraView autofocus="on" ... onTouchEnd={handleTap} />` without any prop that conveys the focus point. Tap-to-focus does not work: the user taps expecting to lock focus to a specific area; the camera continues autofocusing with its default behaviour. Either wire `focusPoint` through the appropriate `CameraView` prop or remove the dead state and `handleTap` function.
...
```
If Claude returns an empty content array (possible on tool-use stop reasons or unexpected API behaviour), `response.content[0]` is `undefined` and `.type` throws `TypeError: Cannot read properties of undefined`. This is caught by the outer `try/catch` and returns a 500 with the raw TypeError message rather than a meaningful error. Fix: guard with `if (!response.content.length || response.content[0].type !== 'text') throw new Error('Unexpected Claude response format')`.

**L15 — `recommendation.topPick` does not exist on `RecommendationResponse`** *(new)*
...
```
`RecommendationResponse` (defined at `src/types/wine.ts:50–53`) has `wines: WineRecommendation[]` and `summary: string`. There is no `topPick` property. `recommendation.topPick` is always `undefined`, so the wine name is never shown on any history card — even in a hypothetical future where scan sessions are actually saved (H1). Fix: replace with `item.recommendation?.wines?.[0]?.name`.

**L16 — Both edge functions reference `claude-opus-4-6`, which is not the current Opus model** *(new)*
`supabase/functions/ocr/index.ts:57,65` and `supabase/functions/recommend/index.ts:169` all specify `model: 'claude-opus-4-6'`. The current Anthropic Opus release is `claude-opus-4-8`. While older minor versions may continue to be served for a grace period, they are subject to deprecation with no advance warning beyond the API changelog. Using the latest version also ensures access to the most capable model and any safety improvements. Update both functions to `claude-opus-4-8`, or parameterise the model via an environment secret so it can be rotated without a code deploy.
