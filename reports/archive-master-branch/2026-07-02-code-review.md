...
**Status:** No application code has changed since the initial commit. All findings from the 2026-07-01 report remain open. This review carries all prior unresolved findings forward with verified file paths and line numbers, corrects one prior false positive (H9), and adds one new finding (S10).
...
No `.catch()` is attached. If AsyncStorage fails (device full, corrupted storage, first boot on some Android versions), the promise rejects silently and `hasLaunched` remains `null` permanently. Line 16 returns `null` while `hasLaunched === null`, leaving the app on a blank screen with no recovery path.
...
The history tab reads from `scan_sessions`. No code anywhere in `app/` or `src/` performs an insert or upsert into this table after a scan completes. Every user's history is permanently empty regardless of how many scans they complete. The entire history feature is structurally broken on the write side.
...
If the auth call throws (DNS failure, unexpected SDK error), `setLoading(false)` is skipped. The button stays in its loading state for the rest of the session.
...
**H9 — RLS INSERT protection on `profiles` and `scan_sessions` — PRIOR FINDING CORRECTED**
`supabase/migrations/001_initial_schema.sql:11–13,26–28`

Previous reports claimed that because no `WITH CHECK` clause was specified, the effective insert check was `WITH CHECK (TRUE)`, permitting inserts with arbitrary `user_id`. **This is incorrect.** PostgreSQL documentation states: *"If no expression is specified in the WITH CHECK clause, a row-level security policy created with just a USING expression will use the USING expression for WITH CHECK purposes."* The `FOR ALL` policy with `USING (auth.uid() = user_id)` therefore applies the same expression as the implicit WITH CHECK on INSERT, correctly preventing authenticated users from inserting rows with a foreign `user_id`. No fix required for this specific claim.

The underlying RLS policies are sound. The findings about `pricing_cache` lacking RLS entirely (S3) and the defence-in-depth concern about the history query not explicitly filtering by `user_id` (noted below under S3) remain valid.

---

**H10 — `scan/preferences.tsx` is missing five required `RecommendInput` fields; enabling route N2 would cause an immediate type error (carry-forward)**
...
`RecommendInput` (`src/services/recommender.ts:5–15`) declares `wineTypes`, `favouriteRegions`, `favouriteGrapes`, `dislikedRegions`, and `dislikedGrapes` as required fields. All five are absent. This is a TypeScript compile error currently concealed only because the route is orphaned (N2). If this screen is wired in, the build fails. At runtime in a non-strict build these being `undefined` silently drops colour restriction, regional exclusions, and grape exclusions.

Fix: read the missing fields from `usePreferences()` and include them in the `recommendWines` call, mirroring `app/scan/extracting.tsx:101–112`.
...
When auth resolves but the preferences query is still in flight, `preferences` is `undefined` (the React Query loading state), not `null`. The guard evaluates to `false` and the user is sent to scan. A brand-new user with no profile row bypasses onboarding on every cold-start launch.

Fix: also destructure and check `isLoading` from `usePreferences`; return `null` while either auth or preferences is loading.
...
`signOut()` returns `{ error }`, which is not checked. If it fails (network error, expired token), navigation still proceeds. The session token remains in SecureStore and the user appears signed-in on the next cold start.
...
No `app/auth/callback.tsx` exists. When the user taps the email-confirmation link from their inbox, Expo Router cannot match the route and the user is silently dropped at the root index with no acknowledgment that the email change succeeded or failed.
...
Neither `handleSignIn` nor `handleSignUp` validates inputs before calling Supabase. Submitting a blank email and password surfaces Supabase's raw internal error string in an `Alert`.

Fix: add explicit guards before calling the auth methods:
```tsx
...
**M15 — `Promise.all` over multiple screenshots holds all base64 payloads in memory simultaneously (carry-forward)**
...
`handleAddCustomDislikedRegion` (line 88) and `handleAddCustomDislikedGrape` (line 96) check `current.includes(trimmed)` but not `current.length >= 5`, unlike the corresponding "likes" handlers at lines 75 and 83. The `ChipPicker` for dislikes is rendered with `max={5}`, but users can bypass this cap by typing into the free-text input field.
...
No `before update` trigger sets `updated_at = now()` on row modification. The upsert in `usePreferences` does not include `updated_at` in the payload.
...
if (hasDuplicateGrapes(parsed.data.wines)) {
  ...
  if (parsed2.success) return parsed2.data; // hasDuplicateGrapes never run on retry result
}
return parsed.data; // original duplicate-grape result returned if retry fails schema validation
```

If the retry still contains duplicates, they are silently returned. If the retry fails schema validation, the original duplicate-grape result is returned instead of throwing.
...
If a Cloudflare 502 or Supabase maintenance page is returned, `JSON.parse` throws a `SyntaxError` whose message propagates as the user-visible error detail.

Fix: wrap `JSON.parse(text)` in `try/catch` and rethrow with a human-readable message.
...
This 196-line component is never imported by any file. `app/scan/results.tsx` reimplements the wine card layout inline. Two divergent implementations exist and must be kept in sync independently.
...
No `default_currency` column exists in any migration. The query does not select it and the returned object never sets it. Any code that reads `preferences.defaultCurrency` receives `undefined`.

---

**L9 — `hasDuplicateGrapes` only splits on `/`; common blend separators not handled (carry-forward)**
...
Common blend notations include `"Cabernet Sauvignon, Merlot"`, `"Cabernet Sauvignon-Merlot"`, and `"Grenache & Syrah"`. The duplicate check produces false negatives for wines sharing a primary variety but using different separators.

Fix: normalise on all four separators — `grape?.split(/[\/,\-&]/)[0].trim().toLowerCase()`.

---

**L10 — Tap-to-focus stores coordinates but never passes them to `CameraView`; feature silently does nothing (carry-forward)**
`app/scan/camera.tsx:15,24–27,103–111`

`focusPoint` state is updated on tap (line 25) but is never passed as a prop to `CameraView`. The camera stays in continuous-autofocus mode regardless of where the user taps.

Fix: add `focusPoint={focusPoint}` to the `CameraView` props and set `autofocus="off"` so focus only locks when the user taps.

---

**L11 — `handleRetake` in preview screen triggers a double navigation to `/(tabs)/scan` (carry-forward)**
`app/scan/preview.tsx:10–12,16–19`
...
function handleRetake() {
  reset();                          // sets imageUri = null
  router.replace('/(tabs)/scan');   // explicit navigation
}
```

`reset()` sets `imageUri` to `null`, which triggers the `useEffect` guard on the next render cycle. Two concurrent `router.replace` calls are dispatched to the same destination.

Fix: remove the explicit `router.replace` from `handleRetake` and rely solely on the `useEffect` guard.
...
`profiles` and `scan_sessions` both have `alter table … enable row level security`. `pricing_cache` does not. Any caller with the anon key can read or overwrite all cached pricing data through the PostgREST API without going through the edge function.

Additionally, the history query at `app/(tabs)/history.tsx:17–22` does not include an explicit `.eq('user_id', session.user.id)` filter, relying entirely on RLS for user-scoping. While the RLS policy is correctly defined (see H9 correction above), omitting an explicit filter removes a defensive layer; a future RLS misconfiguration or policy gap would silently expose all sessions to every authenticated user.
...
The recommend prompt injects `£${budget}` regardless of currency. The results screen always renders `£{wine.menuPrice}`. The OCR function correctly extracts a `currency` field, but this is ignored downstream. EUR and USD menus receive a budget constraint and price display in the wrong currency.
...
The Supabase client resolves with `{ data, error }` rather than throwing. Because the result is not destructured, any error (RLS rejection, network failure, schema mismatch) is silently discarded. `onError` at line 50 is never invoked because the `mutationFn` resolved without throwing. Users see no feedback when preferences fail to save.
...
No `AbortSignal.timeout(...)` is passed. A slow or unresponsive URL hangs the Deno function until Supabase's wall-clock limit kills it (typically 60 s), burning the entire function budget.
...
The system prompt instructs the model to assess drinking windows "as of today's date", but no actual date is injected into the request. The model uses its internal sense of "now" from training data. A wine assessed as "Approaching" peak at training cut-off may actually be at "Peak" or "Fading" by the time the user scans.

Fix: inject today's date into the user message — add `Today's date: ${new Date().toISOString().slice(0, 10)}.` to the `userContext` block at line 150, before the wine list.

---

**S10 — Wine-Searcher API key passed as a query-string parameter; appears in server logs (new)**
...
Query-string parameters appear in HTTP server access logs, CDN logs, and any logging middleware on the Wine-Searcher side. If Wine-Searcher's API supports an `Authorization` header or an `api_key` POST body field, prefer that. If the query-string is the only supported auth mechanism, this is unavoidable but should be documented.
...
`<TouchableOpacity style={styles.card}>` has no `onPress`. Users receive the visual press affordance with no result. Replace with `<View>` until a detail route is implemented, or add `onPress` with navigation to a recommendation detail screen.
...
`router.push` adds a new entry to the history stack. The back-arrow icon implies popping the stack. Replace with `router.back()` or remove the button entirely since tabs handle their own navigation.
...
When `stage === 'recommending'`, line 148 renders "Scoring by critic rating, vintage quality and value" and lines 150–152 render "This may take a minute or two" as a second unconditional `<Text>` element — both visible at the same time. Remove the duplicate copy.
...
None of these screens use `SafeAreaView` or `useSafeAreaInsets`. On iPhones with Dynamic Island the capture button and top content can be obscured. `app/(tabs)/scan.tsx:181` and `app/(tabs)/profile.tsx:449` hardcode `paddingTop: 96`, which is insufficient on newer devices and excessive on older notchless devices.
...
No `router.push('/scan/preferences')` or `href="/scan/preferences"` exists in any file. The screen is unreachable from any navigation path. See also H10 — the screen contains a type error that would be exposed if it were wired in. Delete this file or fix and wire it into the scan flow.
...
`app/scan/extracting.tsx` — see U7. Once extraction starts, back gesture and hardware back are the only escapes, and both leave the scan store in a partially populated state.
...
*Automated review — 2026-07-02. No application code has changed since the initial commit. All findings except H9 (corrected false positive) and S10 (new) are carried forward from prior reports and remain unresolved.*
