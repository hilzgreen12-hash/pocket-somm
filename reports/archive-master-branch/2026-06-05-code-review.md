...
Zero issues from the 2026-06-04 report have been resolved. No code commits exist since the initial codebase was created; all carry-forward findings remain open. Three new issues are identified in this pass.
...
**H7 — `handleEmailChange` in profile leaves `emailSaving` permanently true if `updateUser` throws** *(new)*
...
```
This is the same pattern as H4, but the previous reports only flagged `sign-in.tsx` and `sign-up.tsx`. If `supabase.auth.updateUser` throws (network timeout, SDK error), `setEmailSaving(false)` is never called. The Confirm button renders an `ActivityIndicator` permanently and the email-change UI is frozen until the app is killed. Fix: wrap in a `try/finally` block with `setEmailSaving(false)` in the `finally` clause.
...
**M13 — Sign-up discards the returned session when email confirmation is disabled** *(new)*
...
**L10 — `profiles.updated_at` is never updated on upserts; column reflects creation time only** *(new)*
